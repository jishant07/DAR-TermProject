...
1. S. Kotsiantis , C. Pierrakeas & P. Pintelas ; Predicting Students' Performance In Distance Learning Using Maching Learning Techniques
...
We also know that poverty has been an obstacle in obtaining quality education. As a domino effect it effects the students ability to perform due to all the other circumstances that come with poverty^6^.  
...
```{r include=FALSE} 
  ...
  student.mat <- subset(student.mat, select = -c(reason,guardian,activities,higher,Dalc,Walc,famrel))
  
  #Segregating the Dataset into two populations
...
```{r out.width="50%", fig.cap="Population distribution between genders", fig.align='center', echo=FALSE}
  ggplot(student.mat, aes(x=sex)) + geom_bar(stat = "count",width = 0.5,fill=primary) #Gives out the Gender Distribution graph 
```
```{r}
...
```{r out.width="50%", fig.align='center', fig.cap="Male Population frequency distribution"}
...
```{r out.width="50%", fig.align='center', fig.cap="Female population frequency distribution"}
...
```{r}
  pulled_df <- as.data.frame(as.data.frame(student.mat)[,c("sex","G1","G2","G3")]) #Making a selective data frame
...
```{r out.width="50%", fig.cap="G1 marks vs G3 Marks with Regression Line", fig.align='center'} 
  ggplot(student.mat, aes(x=G1, y=G3)) + geom_point(color=primary) + 
  geom_smooth(method='lm',color=third)
```
```{r out.width="50%", fig.cap="G2 marks vs G3 Marks with Regression Line", fig.align='center'}
...
_We were able to understand that there are a lot of factors to be considered that have a direct or an indirect influence on the marks of the student._
